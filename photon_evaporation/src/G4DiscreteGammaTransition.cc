...
void G4DiscreteGammaTransition::SelectGamma()
{
  ...
  if (nGammas > 0)
	...
	_gammaEnergy = _excitation; 
      ...
      // Evan Rand - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      _levelEnergy      = (_level.Energy());
      _a2               = (_level.A2())[iGamma];
      _a4               = (_level.A4())[iGamma];
      _maxWTheta        = (_level.MaxWTheta())[iGamma];
      _higherLevelEnergy= (_level.HigherLevelEnergy())[iGamma];
      //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
G4double G4DiscreteGammaTransition::GetThetaFromWTheta(G4double higherLevelEnergy)
{
  ...
  if(higherLevelEnergy<0) {
    ...
    while (!foundTheta) {
      ...
      WTheta = (1+_a2[hlevelindex]*LegendreP(2,std::cos(theta))+_a4[hlevelindex]*LegendreP(4,std::cos(theta)))*std::sin(theta);
      if(normy <= WTheta) {
        value = theta;
        foundTheta = true;
      }
