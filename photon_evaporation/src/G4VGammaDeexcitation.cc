...
G4Fragment* G4VGammaDeexcitation::GenerateGamma()
{
  ...
  if(G4NuclearLevelStore::GetInstance()->GetUserFilesMultipole(_nucleus->GetZ(), _nucleus->GetA())) { // if we have given the multipole file
      firstGammaDecay = G4NuclearLevelStore::GetInstance()->GetFirstGammaDecay();

      if(firstGammaDecay) { // is this the first gamma decay?
          // This level was beta fed.
      ...
      //G4cout << "GetUserFilesMultipole = false " << G4endl;
      cosTheta = 1. - 2. * G4UniformRand();
      sinTheta = std::sqrt(1. - cosTheta * cosTheta);
      phi = twopi * G4UniformRand();
  }

  // if the transition was not a gamma, ie IC electron, send it off isotropically
  if (G4NuclearLevelStore::GetInstance()->GetUserFilesMultipole(_nucleus->GetZ(), _nucleus->GetA()) && !(dtransition->IsAGamma()) ) {
      cosTheta = 1. - 2. * G4UniformRand();
      sinTheta = std::sqrt(1. - cosTheta * cosTheta);
      phi = twopi * G4UniformRand();
  }
